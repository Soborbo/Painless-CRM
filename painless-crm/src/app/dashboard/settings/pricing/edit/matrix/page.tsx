import { PricingMatrixForm } from '@/components/domain/pricing/pricing-matrix-form';
import { requireRole } from '@/lib/auth/require-role';
import { getActivePricingVersion } from '@/lib/queries/pricing';
import { getTranslations } from 'next-intl/server';
import Link from 'next/link';

export default async function PricingMatrixEditPage() {
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-10">
      <header>
        ...
        <p className="mt-1 text-sm text-[var(--color-muted-foreground)]">{t('matrixSubtitle')}</p>
      </header>

      {active ? (
        <PricingMatrixForm
          active={{ version_label: active.version_label, config: active.config }}
        />
      ) : (
