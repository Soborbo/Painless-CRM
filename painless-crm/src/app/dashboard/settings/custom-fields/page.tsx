...
import { getCustomFieldDefs } from '@/lib/queries/custom-fields';
import { getTranslations } from 'next-intl/server';
import { AddCustomFieldForm } from './add-form';
import { DeleteDefButton } from './delete-button';

const ROLES = ['admin', 'super_admin'] as const;

export default async function CustomFieldsPage() {
  await requireRole(ROLES);
  const [defs, t] = await Promise.all([getCustomFieldDefs(), getTranslations('customFields')]);
          ...
          <ul className="mt-2 flex flex-col divide-y rounded-md border">
            {defs.map((def) => (
              <li key={def.key} className="flex items-center justify-between gap-3 p-3 text-sm">
                ...
                <DeleteDefButton fieldKey={def.key} />
              </li>
            ))}
          </ul>
        )}
