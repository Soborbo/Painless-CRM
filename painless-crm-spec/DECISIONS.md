...
- **Trigger via the automation engine (ADR-024), not bespoke.** A new `action_type='calendar_sync'` in `automation-cron.ts` (currently `send_email`/`create_task`), riding existing `job.stage_changed`: ENTER `survey_scheduled` → Surveys event; ENTER `accepted` (where `move_date` is first required + locked, STATE_MACHINE §3) → Moves event, re-`patch`ed on `confirmed`; ENTER `cancelled`/`declined` → delete. Re-sync on brief/move-date edits via best-effort enqueue from the relevant mutations (existing `automation_queue` + per-minute cron).
**Alternatives considered:**
