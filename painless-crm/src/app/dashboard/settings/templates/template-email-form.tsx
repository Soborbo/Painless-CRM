...
import { type TemplateActionState, saveEmailTemplate } from '@/lib/actions/templates';
import { extractVariables, renderTemplate } from '@/lib/comms/render';
import type { EmailTemplateRow } from '@/lib/queries/templates';
import { useActionState, useState } from 'react';

const INITIAL: TemplateActionState = { status: 'idle' };

// Shows {{variables}} marked as «name» so the editor sees the merge structure.
function preview(text: string): string {
  const vars = extractVariables(text);
  return renderTemplate(text, Object.fromEntries(vars.map((v) => [v, `«${v}»`])));
}

export function EmailTemplateForm({ template }: { template?: EmailTemplateRow }) {
  const [state, action, pending] = useActionState(saveEmailTemplate, INITIAL);
  const [subject, setSubject] = useState(template?.subject_template ?? '');
  const [body, setBody] = useState(template?.body_template ?? '');
  const vars = [...new Set([...extractVariables(subject), ...extractVariables(body)])];
    ...
    <form action={action} className="flex max-w-2xl flex-col gap-4">
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <input
          ...
          placeholder="quote_followup, invoice_reminder…"
          className="rounded-md border px-3 py-2"
        />
      </label>
      <label className="flex flex-col gap-1 text-sm">
