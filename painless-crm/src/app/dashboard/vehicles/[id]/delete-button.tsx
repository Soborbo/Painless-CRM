...
import { type VehicleActionState, softDeleteVehicle } from '@/lib/actions/vehicles';
import { useTranslations } from 'next-intl';
import { useActionState } from 'react';

const INITIAL: VehicleActionState = { status: 'idle' };

export function DeleteVehicleButton({ id, version }: { id: string; version: number }) {
  const t = useTranslations('vehicles');
  const [state, action, pending] = useActionState(softDeleteVehicle, INITIAL);
