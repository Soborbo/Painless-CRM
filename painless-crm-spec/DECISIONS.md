...
## ADR-040 — Configurable per-user, per-event email notification subscriptions
**Date:** 2026-06-13
**Status:** accepted
**Context:** A go-live request — "every morning at 09:00 UK time, email Richard a summary of website-calculator leads worth more than £800 who did not request a callback" — would have been a one-off hard-coded cron. The better generalisation: let any staff member subscribe to any business event at a chosen cadence. The Phase-15 in-app notification system already had the spine (a `notifications` table, a `notification_preferences` table, event producers, a daily/weekly digest cron) but only a single global `email_digest_enabled` on/off toggle, weekday-only, emailing every notification regardless of type. The Comms Hub automation engine (ADR-024) is for *customer-facing* email, so it was the wrong seam for *internal-staff* email.
**Decision:** Extend the notification system into a **per-user × per-event-type × frequency** model.
- **Event catalog** (`src/lib/notifications/events.ts`) — config-as-data (ADR-034 pattern): ~27 events grouped Sales/Ops/Finance/Care/Collab, each with a stable key (also `notifications.type`), bilingual labels, a `scope` (`broadcast` = fan out to subscribers; `targeted` = intrinsic recipient like assignee/mention), and a `defaultFreq`. Legacy type strings (`assignment`, `sla_breach`, `review_arrived`, …) map to catalog keys via `normaliseEventKey`, so existing producers keep working unchanged.
- **Frequencies**: `off | immediate | hourly | daily | weekly`, stored per user in `notification_preferences.event_prefs` jsonb (migration 56). Unset events inherit the catalog default. The existing `email_digest_enabled` is retained as a per-user **master kill-switch**.
- **Delivery** is decided by the recipient's effective frequency, run by four HMAC-guarded crons: `notify-immediate` (*/5m, also the janitor that stamps off/undeliverable rows), `notify-hourly` (hourly), `daily-digest` (refactored), `notify-weekly`. `notifications.email_sent_at` (migration 56) is the single idempotency guard — every path claims `where email_sent_at is null` and stamps it, so overlapping crons never double-send.
- **09:00 UK time** without per-season config: the daily/weekly crons fire at both 08:05 and 09:05 UTC; the pure `planLondonFlush(now)` guard (Intl `Europe/London`) lets only the run where London local time is 09:00 proceed (BST in summer, GMT in winter). The :05 offset avoids the existing `0 9` sla-digest slot.
- **The £800 case** is one event (`lead.high_value_uncontacted`), scanned daily before the digest sweep. "Did not request a callback" = absence of a `phone_calls` row with `outcome='callback_requested'` — confirmed there is **no** customer-facing callback flag in the schema. Threshold is `settings.notification_settings.high_value_lead_threshold_pence` (default 80000 = £800). Deduped against already-notified jobs like the SLA-breach pattern.
- **Admin reach**: a user always edits their own subscriptions; managers/admins can edit any user in their company (the matrix UI's user picker + a role + same-company check in `updateEventPrefs`, which writes via the admin client).
**Alternatives considered:**
- Hard-code the single £800 daily report → rejected; the request is one instance of a general need, and a matrix is barely more code than a bespoke cron + query.
- Drive subscriptions through the Comms Hub automation engine → rejected; that engine targets customers and has no per-internal-user fan-out or in-app row. The two systems stay separate (customer email vs staff email).
- One time-aware "mega-dispatch" cron instead of four routes → rejected; the project convention is one route per cron, and per-bucket routes keep each HMAC payload and the London guard simple and independently testable.
- A new dedicated email-sent ledger table → rejected; a nullable `email_sent_at` column on `notifications` is sufficient and indexed partially for the frequent sweep.
**Consequences:** Requires migration 56 applied before the feature works (code + migration deploy together, same rule as ADR-037/039); `database.types.ts` carries the three new columns ahead of regen. `schedule.ts` and `wrangler.toml` gained six cron expressions and must stay in lockstep. Legacy cron producers were left untouched (the type-mapping covers them); wiring `complaint.*` fully is deferred (the catalog exposes them and the existing complaint email path is unaffected). Cloudflare cron firing + live Resend send remain infra-gated, so the feature is verified by pure-unit + cron-route-guard tests, not a local end-to-end send.

## ADR-041 — Tamar Call Stats (CDR) integration + inbound call inbox
**Date:** 2026-06-16
**Status:** accepted
**Context:** The main Painless number is hosted at Tamar Telecommunications, who enabled a Call Stats API for us. Tamar exposes a *pull-based* CDR API (`/cdrs/summary`, `/cdrs/?number=&start=&end=`) — there is no inbound webhook. The ask: see who called the main line, recognise the caller against existing customers (and open the right job), count repeat calls, let a logged-in user mark "I called this person back" with a note that lands on the job, and create a job on the spot when the caller is unknown. Phase 02 already shipped a `phone_calls` table whose `source` enum reserved `tamar_api`, plus E.164 normalisation (`normalizePhone`), the calls panel, and the job timeline — so most of this is ingestion + one new view, not new spine.
**Decision:** Poll-and-ingest, reusing existing rails.
- **Ingestion**: an HMAC-guarded cron (`/api/cron/tamar-poll`, scheduled `2-59/5 * * * *` ≈ every 5 min) calls `runTamarPoll`. Tamar's `/cdrs` filters by **date only (`dd/mm/yyyy`)**, so each run re-fetches yesterday+today (spans midnight) and upserts each CDR into `phone_calls` (`source='tamar_api'`). Re-fetching the whole day means **idempotency is mandatory** → migration 58 adds `phone_calls.external_id` (the CDR `uuid`) with a unique index on `(company_id, source, external_id)`; NULLs stay distinct so the many manual rows never collide. Tamar uses **HTTP Basic** auth (`base64(login:token)`).
- **Mapping (locked to the real API manual, 2026-06-16)**: `mapCdrToPhoneCall` reads each logical field from a candidate-key list (kept tolerant for drift) but the real fields are now known — `uuid`, `start` ("YYYY-MM-DD HH:MM:SS"), `caller`/`called` (already E.164), `duration` (decimal seconds), `call_result` ("Answered"/"Not answered"/"Engaged"). Missed = inbound && `call_result != "Answered"` (falls back to zero talk-time). Caveat: `start` has no offset → parsed as runtime-local (UTC on Workers), so stored instants may be off by the BST hour; refine with an explicit Europe/London conversion if exact wall-clock matters.
- **Matching**: caller → customer by exact match on `phoneMatchVariants` (E.164 + national `0…` + `44…`) against `customers.primary_phone`, then → that customer's most-recent live job. Repeat-call count is grouped in a pure helper over the inbox window.
- **Call-back attribution**: migration 58 adds `returned_at` + `returned_by_id`. `markCallReturned` stamps both from the authenticated user (login-attributed) and writes the optional note to the linked job (`notes.parent_type='job'`, category `staff`) so what happened on the call shows on the job sheet + timeline for everyone — chosen over logging a separate outbound call row (one click, one source of truth on the job).
- **Create-from-call**: `createJobFromCall` find-or-creates the customer from the caller number and opens a `lead` job (mirrors `createJob`: round-robin assign, job-number retry, status history, lead.created event), then links the call.
- **Config in env** (non-OAuth creds, per ADR-009/rule 16): `TAMAR_API_LOGIN` + `TAMAR_API_TOKEN` (Basic auth), `TAMAR_API_BASE`, `TAMAR_NUMBERS`; tenant via `WEBHOOK_COMPANY_ID`. Every gate degrades to a typed no-op (`no_api_key`/`no_numbers`/`no_company`) so the cron is safe to register before go-live.
- **Missed-call notification (ADR-040)**: a new broadcast catalog event `call.missed` (Sales group, default `immediate`) fires from ingestion for each genuinely-new (`seen` set) inbound missed CDR, linking to the matched job or the call inbox. `event_prefs` (notification subscriptions) is already live on prod, so it delivers.
**Alternatives considered:**
- Wait for a Tamar webhook → rejected; the API is pull-only.
- A dedicated `calls`/`cdr` table → rejected; `phone_calls` already models direction/numbers/duration/outcome/customer/job and feeds the timeline. Two columns + an index beat a parallel table.
- Separate outbound row for each call-back → rejected for the stamp+note model (see above); the action could still be extended to log a full outbound call later via the existing `logPhoneCall`.
- Fuzzy phone matching (trigram) → rejected for v1; exact-match on a few normalised variants is index-friendly and predictable. Unmatched calls are first-class (the inbox shows "No match" + Create job).
**Consequences:** Migration 58 **applied to live prod 2026-06-16** (via MCP, recorded `tamar_call_log`); `database.types.ts` carries `external_id`/`returned_at`/`returned_by_id` ahead of regen. The `2-59/5` cron is registered in `wrangler.toml` + `schedule.ts`, but Cloudflare-side activation + the `TAMAR_API_LOGIN`/`TAMAR_API_TOKEN`/`TAMAR_NUMBERS` env still need a deploy before any call appears — until then the inbox shows manually-logged inbound calls. The CDR mapping is now locked to the real API manual (no longer provisional), but live polling can't be exercised on native Windows, so the feature is verified by pure-unit tests (mapping incl. real Tamar shape, missed/answered, number parsing, date format, match variants, repeat counts, action schemas, `call.missed` catalog) + the cron-route-guard pattern, not a local end-to-end fetch.

## ADR-042 — Unified tasks entity (callbacks + job checklist + follow-ups)
**Date:** 2026-06-16
**Status:** accepted
**Context:** Day-to-day work was tracked in two disjoint to-do systems and a gap. (1) `job_tasks` — a per-job checklist (Phase 19 / ADR-028), visible only on the job page, no priority, no standalone view. (2) Call-backs — `phone_calls.next_action*` columns surfaced on a dedicated queue + home banner, tied to a phone call. (3) Missing entirely: the contact-centric, prioritised follow-up list the legacy iMVE task screen provided ("FU for survey", "chase quote"), linked to a person and assignable. Staff had to look in two places and could not raise a general follow-up against a customer/quote/complaint.
**Decision:** One `tasks` table is the single actionable-task entity (migration 59).
- **Polymorphic link**: `related_type` ∈ {customer, job, quote, complaint, phone_call} + `related_id`, with denormalized `customer_id`/`job_id` for fast queue + entity-panel reads.
- **`kind` discriminator** {followup, checklist, call} keeps three micro-UXes on one table: the inline job checklist (`checklist`, ordered by `sort_order`), the call-back queue (`call`), and general follow-ups (`followup`). It adds what the old systems lacked: `priority` {low,medium,high}, `status` {open,in_progress,done,cancelled}, and a datetime `due_at`.
- **Multi-assignee** (chosen now, not deferred): a `task_assignees` join table — a task carries 0..N assignees. "My tasks" resolves the caller's ids through the join, then loads full rows with every assignee. The add/reassign controls are native multi-selects.
- **Backfill + drop `job_tasks` now** (chosen over a dormant-then-drop release): migration 59 backfills open call-backs (→ `kind='call'`, assignee = the call's `user_id`) and every live `job_tasks` row (→ `kind='checklist'`, preserving `sort_order`/`done`/assignee) into `tasks`, then `drop table job_tasks`. The job checklist UI is re-pointed at `tasks` in the same PR. `phone_calls.next_action*` columns are kept as call history; `tasks` now owns the actionable queue.
- **Surfaces**: a central `/dashboard/tasks` worklist (scope mine/all, open-only/all, sorted overdue→today→priority→soonest by the pure `sortQueue`); a reusable `TaskQuickAdd` to drop on any detail page with context pre-filled; a "tasks due today" home tile; nav link; best-effort `task.assigned` in-app notification to each new assignee.
**Alternatives considered:**
- Keep three systems and just add `priority` to `job_tasks` → rejected; the fragmentation (two queues, no general follow-up) was the actual complaint.
- A `tasks` table parallel to call-backs → rejected; that leaves two queues. Call-backs are folded in as `kind='call'`.
- Single `assigned_to_id` with a join table later → rejected per this ADR; the iMVE screen assigns to multiple staff, and the join is cheap now.
- Keep `job_tasks` dormant for one release → rejected per this ADR; backfilling and dropping in one migration avoids a second migration and a stale table, at the cost of doing the UI re-point in the same PR.
**Consequences:** Requires migration 59 applied before deploy (code + migration together, same rule as ADR-037/039/040/041). The Supabase client is not bound to the generated `Database` generic (see `supabase/server.ts`), so `database.types.ts` did not need regen for the new tables. Notification fan-out is best-effort (single targeted insert, no event-catalog entry yet); wiring `task.assigned`/`task.due` into the subscription matrix + a due/overdue digest cron is deferred. Also deferred (additive, no schema change): the automation engine's `create_task` action, `TaskQuickAdd` on the customer/quote/complaint detail pages, the call-back queue's repoint onto `tasks`, a Cmd+K "New task" command, and the calendar overlay. Verified by pure-unit tests (dueBucket/sortQueue/completeness + all schemas); live RLS + the backfill are validated on migration apply, which is infra-gated.

## ADR-043 — Compare My Move inbound lead webhook (v3 body-signed handler)
**Date:** 2026-06-16
**Status:** accepted
**Context:** Compare My Move (CMM) refers removals/clearance leads to matched partners. Their Lead Manager delivers each lead to a partner-supplied HTTPS endpoint as an HTTP POST. Three lead types share one envelope: `residential`, `small-move`, `clearance`. The auth scheme is **not** our first-party one: CMM puts `timestamp`, `token`, `signature` in the JSON **body** and signs `HMAC-SHA256(timestamp + token)` under a shared secret — it does not (and cannot be configured to) send our `x-webhook-{signature,timestamp,version}` headers or sign the raw body. CMM also retries a failed delivery up to 6× over ~160 min with the *same* timestamp/token, and only stops early on a `406`.
**Decision:** Add a sibling **v3 body-signed webhook shell** (`lib/webhooks/handler-v3.ts`, `createBodySignedWebhookHandler`) rather than modify the protected v2 shell (`handler.ts`, DO-NOT-MODIFY) or write a bespoke per-route handler (rule 14). v3 reuses v2's constant-time `verifyHmac` (imported, not copied) and the same `webhook_events` table for dedup + audit, so the audit trail and idempotency guarantee are identical. Per-route it takes an `envelope()` that pulls `{signedString, signature, payload, eventId}` out of the body. CMM specifics:
- **Auth**: `signedString = `${timestamp}${token}``; compare `verifyHmac(secret, signedString, body.signature)`. Secret in `CMM_WEBHOOK_SECRET` (env, non-OAuth, ADR-009). Absent ⇒ `503 webhook_disabled`.
- **No timestamp-freshness gate** (deliberately): v2's 300 s replay window would reject every CMM retry and lose the lead. Idempotency instead comes from dedup on `quote_id` (the `webhook_events` UNIQUE(source, event_id)); a retry of an already-ingested lead acks `200 {duplicate:true}`.
- **Lenient schema** (`CmmResultSchema`, `.passthrough()`): CMM warns payloads vary by split-test, so only `quote_id`/`email`/`phone` are required; everything else is nullish and `.passthrough()` keeps unforeseen fields on `webhook_events.payload`. Lossless curated map under `intake_details.compare_my_move`.
- **Normalisation**: `compare_my_move` added to `ACQUISITION_SOURCES` (a `text` column — **no migration**) so CMM is its own attribution channel (matches the existing Lead-Provider settings example, ADR-035). `lead_type` → `service_type` (`clearance`→`waste_clearance`, else `removal`). current→new map to from/to `job_addresses` (clearance = single `from`); flat lift/floor/property-type carried per-leg. Tenant is server-side `WEBHOOK_COMPANY_ID`, never a body value (audit H2).
- **JSON over XML**: CMM offers either; we configure JSON (the route only parses JSON).
**Alternatives considered:**
- Generalise v2's `createWebhookHandler` with a pluggable auth strategy → rejected for now; `handler.ts` is security-critical DO-NOT-MODIFY and the header gates don't map to CMM. v3 keeps v2 untouched and provably unchanged.
- Bespoke handler in the route → rejected (rule 14: no bespoke handlers; loses shared dedup/audit).
- Enforce timestamp freshness like v2 → rejected; breaks CMM's documented retry semantics.
- Strict schema per lead type (discriminated union) → rejected; CMM explicitly does not guarantee payload consistency, so lenient-and-lossless is safer than reject-and-retry.
**Consequences:** No migration. Go-live needs `CMM_WEBHOOK_SECRET` + `WEBHOOK_COMPANY_ID` set and the endpoint URL registered in CMM's Lead Manager (JSON, one of CMM's egress ports 443/444/80/8088/8888). Until then the route returns `503`. Verified by pure-unit tests (schema accepts all three documented shapes + requires the core fields + passthrough; envelope extraction; `verifyHmac` round-trip against a Node `createHmac` oracle; service-type/full-name/address/notes/intake-detail mapping over the doc example payloads) — live delivery can't be exercised on native Windows. Edge case: a `406`-style "give up" response is not emitted; a genuinely unparseable payload returns `400` and CMM retries 6× then gives up (harmless). `INTEGRATION_CONTRACTS.md` carries the field contract.

## ADR-044 — Gmail inbound-mail ingestion (service account + domain-wide delegation)
**Date:** 2026-06-18
**Status:** accepted
**Context:** The shared Painless mailbox (e.g. `info@painlessremovals.com`) on Google Workspace receives leads and customer replies, but none of it reaches the CRM — the customer history is blank on the email side and there is no CRM-side signal when an enquiry lands. The ask mirrors the Tamar call inbox (ADR-041): pull inbound mail in, recognise the sender against existing customers (create a contact when unknown), drop it on the customer timeline, and fire a notification. Gmail offers a push channel (Pub/Sub `watch`) but that needs a Cloud Pub/Sub topic + a public push endpoint + a watch-renewal cron; a pull poll reuses the exact Tamar rails we already run.
**Decision:** Poll-and-ingest, mirroring the Tamar CDR poller 1:1.
- **Auth = service account + domain-wide delegation (DWD).** A GCP service account is authorised over the `gmail.readonly` scope in Workspace Admin; the cron mints an RS256 JWT (`iss`=SA, `sub`=impersonated mailbox, `scope`=gmail.readonly) with Web Crypto (`crypto.subtle`, the same primitive `schedule.ts` HMAC uses) and exchanges it at Google's token endpoint for a short-lived access token (cached in-isolate, refreshed 60 s before expiry). No OAuth consent screen, no refresh-token storage, no Google app verification.
- **Why env, not `integration_credentials` (rule 16 / ADR-009):** the SA private key is a *static service credential* — one key for the whole integration, like `RESEND_API_KEY` — not a per-user OAuth grant issued/refreshed/revoked per account. Rule 16 / ADR-009 governs OAuth *tokens*; an API key lives in env. So `GMAIL_SA_PRIVATE_KEY` (secret) + `GMAIL_SA_CLIENT_EMAIL` are secrets and `GMAIL_MAILBOX` / `GMAIL_BACKFILL_DAYS` sit in `[vars]`. Recorded here so the choice is not re-litigated against rule 16.
- **Portable clean core:** `auth.ts` / `client.ts` / `parse.ts` import neither Supabase nor env (only Web Crypto + fetch), so the Gmail core can later be lifted into the sister szello D1 app unchanged; the Supabase sink (`ingest.ts`) + the env-reading orchestrator (`poll.ts`) are Painless-specific. No cross-project plumbing is built now (the two CRMs stay separate projects — Painless on Supabase, szello on D1).
- **Ingestion:** an HMAC-guarded cron (`/api/cron/gmail-poll`, scheduled `4-59/5 * * * *` ≈ every 5 min, offset off the `*/5` notify-immediate slot, one route per cron expression) calls `runGmailPoll`. With a stored cursor it does an incremental `users.history.list` delta; on a first run — or a 404 "historyId too old", since Gmail purges history after ~1 week — it backfills `newer_than:{GMAIL_BACKFILL_DAYS}d` and re-seeds the cursor from `users.getProfile`. Migration 60 adds `email_sync_state` (the per-(company, mailbox) cursor) and `email_messages` (the mail log), unique on `(company_id, gmail_msg_id)` so re-polling never duplicates (upsert ON CONFLICT DO NOTHING + a "seen" set guarding the note/notification side-effects, exactly like the Tamar ingest).
- **Matching:** from-address → customer by case-insensitive exact match on `customers.primary_email`; an unmatched *inbound* sender creates a lightweight `individual` contact (name split from the From display name, `acquisition_source='other'`). The message is summarised onto the customer timeline (`notes.parent_type='customer'`, category `admin`).
- **Direction:** classified in the pure parser — From ∈ the tenant mailbox(es) ⇒ outbound, else inbound. Round 1 acts only on inbound (matching / contact-create / notify); outbound rows are stored with the correct direction but do not notify.
- **`email.received` notification (ADR-040):** a new broadcast catalog event (Sales group, default `immediate`) fires from ingestion for each genuinely-new inbound message, linking to the matched/created customer.
**Alternatives considered:**
- Gmail push (Pub/Sub `watch`) → rejected for round 1; needs a Pub/Sub topic, a public authenticated push endpoint and a 7-day watch-renewal cron. Polling reuses the Tamar pattern with zero new infra and is idempotent.
- Per-user OAuth via `integration_credentials` → rejected; this is one shared mailbox read by a service, not N user inboxes — DWD is Google's recommended server-to-server path and avoids a consent/verification flow.
- A dedicated push / IMAP worker → rejected; the cron + a portable core is fewer moving parts and matches ADR-041.
- Resend Inbound / Cloudflare Email Workers parsing forwarded MIME (the old OD-1 Tamar-email idea) → rejected for Gmail; we have first-party API access to the mailbox, so structured `messages.get` beats parsing forwarded mail.
**Consequences:** Requires migration 60 applied before deploy (code + migration together, same rule as ADR-037/039/040/041). The Supabase client is not bound to the generated `Database` generic, so `database.types.ts` needs no regen for the new tables. Go-live needs the one-time GCP/Workspace setup (enable Gmail API; create the SA + JSON key; authorise the SA's numeric client-ID for `gmail.readonly` in Workspace Admin → Security → API controls → Domain-wide delegation) plus the `GMAIL_SA_CLIENT_EMAIL` / `GMAIL_SA_PRIVATE_KEY` secrets + `GMAIL_MAILBOX`; until then `runGmailPoll` degrades to a typed no-op (`no_credentials` / `no_mailbox` / `no_company`) and the route still returns 200. Like Tamar, the cron fires in prod only once the `worker.ts` scheduled handler is the deployed entry (shared, already-pending infra gate) — so the feature is verified by pure-unit tests (RS256 JWT sign/verify against a generated keypair, header/body/base64url parsing, direction classification; dedup + match-vs-create + note + `email.received` in the ingest; first-run / delta / 404-fallback / no-op in the poll) + the cron-route-guard pattern, not a live fetch. Outbound capture and thread stitching (`In-Reply-To` / `Message-ID` are already stored for it) and the szello port are deferred. `INTEGRATION_CONTRACTS.md` carries the field contract.

## ADR-045 — Calendar sync: Google Calendar API two-way push, role-segmented calendars, structured move-brief
**Date:** 2026-06-19
**Status:** accepted
**Context:** Jay's field crews cannot see job details on the road. iMVE imports only date + time to a calendar that only Jay and Tom can see; the van team gets "where + what time" with no customer phone, no access notes, no dismantle/kit information. The ask: when a survey is booked and again when move dates are agreed, push the job to a calendar the whole crew already has on their phones, carrying the *full brief* — customer name + number, from/to addresses with access/property details, dismantle + reassembly, the kit to bring (protectors, mattress/TV bags), notes, and the "not going" list. The CRM already has (a) the `accepted` quote-acceptance flow the ask describes (`quote/[token]/accept-form.tsx`, `lib/actions/quote-acceptance.ts`) and (b) a Google service-account + domain-wide-delegation (DWD) auth primitive shipped for Gmail (ADR-044, `integrations/gmail/auth.ts`) whose `getAccessToken(creds)` is already scope-parameterised. So this is an *extension of an existing integration*, not green-field.
**Decision:** Build a Google Calendar **two-way push** over one shared pure core, in role-segmented calendars, with structured fields for the missing brief data (the three product decisions taken with Laszlo on 2026-06-19).
- **Pure core (reused by every sink, no I/O, unit-tested — cf. `lib/dispatch/board.ts`, `lib/calendar/grid.ts`):** `lib/calendar/brief.ts` `assembleJobBrief(input, audience)` gathers the scattered data (job, `job_addresses`, `surveys`, `cubic_sheet_items` dismantle/reassembly, the new `job_brief_items`, staff/admin `notes`, open `tasks`, `arrival_window`) into a typed `JobBrief`, filtered by **`audience: 'crew' | 'customer'`** (the PII boundary — crew sees everything; a customer brief is sparse). `lib/calendar/event.ts` maps a brief to a Google event resource (and later an RFC-5545 VEVENT, hand-rolled, no new dep — cf. the CSV modules ADR-033).
- **Auth = reuse ADR-044, no modification of the protected file.** `integrations/google-calendar/auth.ts` *imports* the exported `getAccessToken` from `gmail/auth.ts` (a `*/auth.ts` DO-NOT-MODIFY file — imported, never edited) with `scope='https://www.googleapis.com/auth/calendar'` and `sub`=the organiser mailbox (DWD, same mechanism as Gmail). Go-live only adds the `calendar` scope to the SA's existing Workspace DWD authorisation. Extracting the shared SA-OAuth primitive to `integrations/google/auth.ts` (anticipated by ADR-044's portable-core note) is a later clean-up that needs human sign-off because it moves a protected file.
- **Two-way push + lifecycle.** `integrations/google-calendar/client.ts` does `events.insert/patch/delete`; `sync.ts` orchestrates assemble→upsert→persist and **degrades to a typed no-op without creds** (the ADR-041/044 pattern — the cron/route still succeeds). Idempotency + update/delete live in a new `calendar_links` mapping table `(entity_type ∈ {survey, job_move}, entity_id, provider, calendar_id, external_event_id, etag, status, last_synced_at)`, unique on `(company_id, provider, entity_type, entity_id)`. Move-date/arrival/address/brief edits → `patch`; `cancelled`/`declined`/survey reschedule → `delete`.
- **Role-segmented calendars (chosen over one shared calendar).** Two calendars — **Surveys** and **Moves** — via env `GOOGLE_CALENDAR_ID_SURVEYS` / `GOOGLE_CALENDAR_ID_MOVES` + `GOOGLE_CALENDAR_ORGANIZER`; the SA secrets reuse the existing `GMAIL_SA_CLIENT_EMAIL` / `GMAIL_SA_PRIVATE_KEY`. v1 is *by role* (two calendars the crew subscribe to), not per-surveyor calendars (deferred).
- **Structured fields now (chosen over config-as-data).** Migration 61 adds: `cubic_sheet_items.reassembly_required boolean` (mirrors `dismantle_required`, per item); a unified **`job_brief_items`** table with a `kind ∈ {kit, excluded}` discriminator (cf. the unified-`tasks` pattern ADR-042) — `kit` = materials to bring, `excluded` = "not going" — deliberately distinct from `job_sheets.materials_used` (the crew's *actual* post-job record). Standard spine: `company_id`, RLS Pattern-1, `set_updated_at`, soft delete, `version`.
- **Trigger via a dirty-flag + drain cron (the gmail/tamar pattern), not the email `automation_queue`.** Calendar sync is a *system* behaviour, not a user-configured rule, so it does NOT ride `automation_rules`/`automation_queue` (which only enqueue matched user rules). Instead producers call best-effort `markEntityDirty(entityType, entityId)` at the relevant points — ENTER `survey_scheduled` (survey); ENTER `accepted` (where `move_date` is first required + locked, STATE_MACHINE §3), `confirmed`, and any move-date / brief edit (job_move); ENTER `cancelled`/`declined` — which upserts the entity's `calendar_links` row to `status='pending'`. A dedicated per-minute cron (`/api/cron/calendar-sync`) drains pending/failed links (`calendar_links_pending_idx`) via `drainCalendarSync` → `syncEntityCalendar` → `runCalendarSync`, so the request path never makes a Google call and a transient failure is retried on the next tick. `buildEntityEvent` rebuilds the event from current DB state and returns null for a cancelled/unscheduled entity, which deletes the event — so create / update / delete all fall out of one drain pass.
**Alternatives considered:**
- **.ics email attachment + subscription feed first, API push later** (the recommended lower-risk path) → not chosen; Jay wants native low-latency two-way updates from the start, accepting the event-lifecycle + mapping-table cost. The pure core is sink-agnostic, so an `.ics` feed/attachment stays a cheap later add (customer-facing) with no rework.
- **One shared "Painless Jobs" calendar** → simpler sharing, but Jay wants survey vs move separated by role.
- **Kit/excluded/reassembly on `jobs.custom_fields` (ADR-034, no migration)** → faster, but Jay wants form-validated, reportable structured fields now.
- **A new auth module / modifying `gmail/auth.ts`** → rejected; the exported `getAccessToken` is already generic and the file is DO-NOT-MODIFY. Import, don't touch.
- **Bespoke per-stage sync calls** → rejected (CLAUDE rule 14 spirit + ADR-024); the automation engine already owns stage-entry side effects with best-effort enqueue + the drain cron.
**Consequences:** Requires migration 61 applied before the feature works (code + migration together, the ADR-037/039/040/041 rule). Go-live needs the one-time Google setup — enable the Calendar API, add the `calendar` scope to the SA's Workspace DWD client-ID, create the Surveys + Moves calendars under `GOOGLE_CALENDAR_ORGANIZER`, subscribe the crew, set the two `calendarId` env vars — until then `runCalendarSync` is a typed no-op (`no_credentials` / `no_calendar` / `no_company`) and stage transitions proceed unaffected. The Cloudflare cron firing remains the shared pending infra gate (`worker.ts` scheduled handler), so the feature is verified by pure-unit tests (brief assembly + audience filter, brief→event mapping, `calendar_links` lifecycle, client against a fetch oracle, the `calendar_sync` action guard) not a live push. The worker-PWA job page (`(worker)/jobs/[id]/page.tsx`, today only number/customer/phone/from-address) is enriched from the same brief so the root "no notes on the van" pain is solved even for crew who don't subscribe to the Google calendar. A customer-facing survey-confirmation email does not yet exist in the CRM (it is an iMVE template) — the customer `.ics` attachment rides on building that and is deferred. `INTEGRATION_CONTRACTS.md` gains a §13 (Google Calendar) before go-live. Per-surveyor calendars and true two-way (read Google → CRM) are out of scope for v1.

## ADR-046 — Customer merge / duplicate resolution
**Date:** 2026-06-21
**Status:** accepted
**Context:** The same customer enters the CRM more than once — web-form lead, phone call logged against a fresh record, a bulk import — and the office ends up with split history: jobs under one record, invoices under another. Creation already runs `find_duplicate_candidates` (migration 10) and lets the rep view a match or force-create, but there has been no way to *resolve* a duplicate after the fact. The feature survey lists "merge / duplicate-resolution tool" as the outstanding Customer-360 gap. A merge touches a spine table (`customers`) and re-points ~18 child tables, so it needs an ADR.
**Decision:** Ship duplicate resolution in two clearly separated halves.
- **Detection (non-destructive, pure, unit-tested):** `lib/customers/duplicates.ts` `findDuplicateClusters(customers)` groups the tenant's base by shared **normalised email OR phone** using union-find, so links are transitive (A↔B by email, B↔C by phone ⇒ {A,B,C}). Email normalises to trimmed-lowercase; phone strips to a digits-only national number (drops a UK `44` country code and the trunk `0`) so `+44 7911 …`, `07911 …` and `447911…` key the same; fragments shorter than 6 digits don't key (avoids over-matching). `listCustomersForDedup()` (RLS-scoped, capped at `DEDUP_SCAN_MAX = 5000`) feeds it; the clusterer runs in-process. Surfaced at `/dashboard/customers/duplicates` (oldest record first = the likely keeper; biggest clusters first), linked from the customer list. Viewable by `sales+`.
- **Merge (destructive, atomic, role-gated):** one SECURITY DEFINER RPC `merge_customers(p_company_id, p_winner_id, p_loser_id, p_loser_version)` (migration 62) does the whole fold in one transaction — re-points every child row (`jobs`, `quotes`-via-job, `quote_acceptances`, `invoices`, `payments`, `complaints`, `customer_signoffs`, `review_requests`, `storage_rentals`, `direct_debit_mandates`, `customer_contacts`, `customer_consents`, `messages`, `phone_calls`, `attributions`, `documents`, `email_messages`, `tasks`, `appointments`, and both ends of `customer_relationships`), inherits any contact field the winner was missing (`coalesce`), and **soft-deletes the loser** (CLAUDE rule 11). The body re-asserts tenant isolation + a `manager+` role gate internally (the record_payment/ADR hardening pattern) so a direct PostgREST caller can't forge a cross-tenant merge; both rows are locked `for update` (lower id first, deadlock-safe) and the loser carries an optimistic-version guard (rule 12). The `mergeCustomers` server action loops the RPC once per selected loser, reading each loser's current version first. Merge UI (pick keeper + confirm) shows only to `manager+`.
**Alternatives considered:**
- **App-side re-pointing (sequential `supabase.from(...).update()` per table)** → rejected: PostgREST gives no multi-statement transaction, so a mid-run failure leaves a half-merged customer. Atomicity *is* the feature, hence an RPC (CLAUDE rule 6 sanctions RPCs in lieu of raw app SQL).
- **Hard-delete the loser** → rejected (rule 11); soft delete keeps the merge recoverable and preserves the audit trail.
- **Fuzzy/name + address similarity scoring** → deferred; exact email/phone keys are high-precision and explainable, the right v1. Postcode/name fuzz can layer on later over the same cluster shape.
- **Auto-merge on detection** → rejected; merging is irreversible-ish and a human must choose the keeper.
**Consequences:** Migration 62 is destructive and could not be executed in the build sandbox, so it is verified by pure-unit tests on the clusterer + typecheck/build only — it **must be applied and exercised on a Supabase branch** (seed winner + loser with jobs/invoices, merge, assert child rows moved and the loser is soft-deleted) before promoting to production, per the code+migration-together rule. The re-point list is safe to run blind because no child table has a UNIQUE constraint on `customer_id` (verified against the schema), so moving rows can't collide; the audit trigger fires per UPDATE automatically. The clusterer is in-process and capped at 5000 — a tenant past that needs a server-side scan (out of scope). `customer_relationships` rows that collapse to a self-reference after the fold are soft-deleted. The winner's `version` advances (one bump regardless of loser count via the field-inherit UPDATE, plus a bump from each child re-point's audit is not applicable — only the customers row bumps), so any open optimistic edit on the winner must reload.
