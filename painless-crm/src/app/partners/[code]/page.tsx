import { getPartnerPortalData } from '@/lib/queries/partner-portal';
import { formatDate, formatPence } from '@/lib/utils/format';

// Phase 16 §2 — public affiliate portal. The [code] in the URL is the partner's
// access secret (no login); the query resolves it to one affiliate and shows
// only that affiliate's commission ledger. No customer PII.
    ...
    return (
      <main className="mx-auto flex min-h-screen max-w-xl flex-col gap-4 px-6 py-16">
        ...
        <p className="rounded-md border px-4 py-3 text-sm text-[var(--color-muted-foreground)]">
          This link is no longer valid. Please contact us for an up-to-date link.
        </p>
      </main>
    );
  }
