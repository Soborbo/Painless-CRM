...
### Phase 25 — Customisation engine (fields / statuses / job-sheet / sign-off / email builder)  ·  larger, mixed  ·  ● 25a+25b+25c+job-sheet BUILT (2026-06-03)
> **25d shipped — Job Sheet customisation (migration 49, ADR-036):** reuses the ADR-034 engine verbatim — `settings.job_sheet_fields` defs + `job_sheets.custom_fields` value map; admin UI `settings/job-sheet-fields`; the worker end-of-job sheet renders the extra fields and carries them through the offline queue; `persistJobSheet` coerces them against the defs at write (lenient for offline replay). en/hu `jobSheetFields` ns, nav link (admin). **Remaining in 25:** visual Email Builder only (existing merge-field template editor suffices for v1). Job-status customisation DEFERRED (locked STATE_MACHINE). Customised-template PDF render 🔒.
> **25b shipped (migration 48):** document-text config (`settings.document_text` — acceptance terms → public acceptance page, sign-off declaration → sign-off screen, quote footer; pure `lib/customisation/document-text.ts`; admin UI). **25c shipped (migration 48):** cubic-calculator presets (`settings.cubic_presets`, pure `lib/customisation/cubic-presets.ts`; admin UI; surfaced as a `<datalist>` with volume autofill on the survey cubic sheet). 993 tests.
